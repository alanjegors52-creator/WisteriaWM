...
void apply_layout(Node *n, int x, int y, int w, int h) {
    ...
    if (canvas_mode) {
        if (!n->is_split) {
            ...
            // Convert the window's world-space position (wx, wy) into a
            // screen-space position using the camera and zoom level.
            // This is the core of true zooming: we shift *where* the window
            // appears on screen, but we never touch its actual dimensions.
            // The illusion of zooming out comes from windows moving further
            // apart on screen as zoom decreases, not from them shrinking.
            int sx = (int)((n->wx - cam_x) * zoom + mid_x);
            int sy = (int)((n->wy - cam_y) * zoom + mid_y);

            // Window dimensions are fixed in world space — they don't scale
            // with zoom. This is exactly how DriftWM handles it: the viewport
            // moves, the windows themselves stay the same physical size.
            int zw = n->w;
            int zh = n->h;
...
void add_window(Window w) {
    ...
    if (canvas_mode) {
        ...
        // Place new windows at the cursor's world-space position so they
        // appear under the mouse regardless of where the camera is pointing.
...
int main() {
    ...
    while (1) {
        ...
        if (ev.type == KeyPress) {
            ...
            for (int i=0; i<bind_count; i++) {
                if (binds[i].ks == ks && (ev.xkey.state & binds[i].mask) == binds[i].mask) {
                    ...
                    else if (strstr(binds[i].action, "toggle_canvas")) {
                        canvas_mode = !canvas_mode;
                        // When leaving canvas mode, reset the camera so tiling
                        // layout picks up from a clean state.
                        if(!canvas_mode){cam_x=0;cam_y=0;zoom=1.0;}
                        apply_layout(tree, 0, 0, sw, sh);
                    }
        ...
        if (ev.type == ButtonPress) {
            ...
            if (ev.xbutton.button == 1 && (ev.xbutton.state & MOD) && n) {
                is_moving = true; active_node = n;
                start_win_x = n->wx; start_win_y = n->wy;
            }
            else if (ev.xbutton.button == 3 && (ev.xbutton.state & MOD) && n) {
                is_resizing = true; active_node = n;
                start_win_w = n->w; start_win_h = n->h;
            }
            ...
            else if (ev.xbutton.button == 4 && canvas_mode) {
                // Zoom in: increase zoom, which causes world positions to map
                // further from the centre — giving the feeling of pulling closer.
                zoom *= 1.1; apply_layout(tree, 0, 0, sw, sh);
            }
            else if (ev.xbutton.button == 5 && canvas_mode) {
                // Zoom out: decrease zoom, world positions pull toward centre —
                // you see more of the canvas without windows changing size.
                zoom /= 1.1; apply_layout(tree, 0, 0, sw, sh);
            }
        }
        ...
        if (ev.type == MotionNotify) {
            ...
            else if (is_moving && active_node) {
                // Convert the mouse delta from screen space back into world
                // space by dividing by zoom, so dragging feels consistent
                // regardless of how far in or out you're zoomed.
                active_node->wx = start_win_x + dx / zoom;
                active_node->wy = start_win_y + dy / zoom;
            }
            else if (is_resizing && active_node) {
                active_node->w = start_win_w + dx / zoom;
                active_node->h = start_win_h + dy / zoom;
            }
            apply_layout(tree, 0, 0, sw, sh);
        }
    }
...
echo "Wisteria WM v1.1 installed — canvas zoom now works like DriftWM :3"
